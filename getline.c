...
#else
		r = _get_next_line(info, buf, &len_p);
#endif
		if (r > 0)
		{
			if ((*buf)[r - 1] == '\n')
			...
			delete_comments(*buf);
			build_my_history_list(info, *buf, info->histcount++);
...
ssize_t get_inputs(info_t *info)
{
	...
	if (len) /* we have commands left in the chain buffer */
	{
		...
		*buf_p = p;				   /* pass back pointer to current command position */
		return (strlen_length(p)); /* return length of current command */
	}
...
/**
 * _get_next_line - gets the next line of input from STDIN
 * @info: parameter struct
